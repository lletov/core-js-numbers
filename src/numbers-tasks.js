...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const angleCos =
    x1 * x2 +
    (y1 * y2) / ((x1 ** 2 + y1 ** 2) ** 0.5 * (x2 ** 2 + y2 ** 2) ** 0.5);
...
function roundToPowerOfTen(num, pow) {
  return parseInt(
    Math.round(num / 10 ** pow)
      .toString()
      .split('.')[0] + Array(pow).fill('0').join(''),
    10
  );
}
...
function toNumber(value, def) {
  return !Number.isNaN(parseInt(value, 10)) ? parseInt(value, 10) : def;
}
...
function getSumOfDigits(num) {
  ...
  const numArr = arr.map((el) => parseInt(el, 10));
...
function getCountOfOddNumbers(number) {
  const arr = [...Array(Math.abs(number) + 1).keys()];
  const counter = arr.map((el) => el % 2 !== 0);
  return counter.filter((word) => word === true).length;
}
